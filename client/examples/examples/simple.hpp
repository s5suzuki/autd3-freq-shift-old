...
class FocalPointGain final : public autd::Gain {
 public:
  static autd::GainPtr Create(const Vector3& point, uint8_t duty = 0xFF) { return std::make_shared<FocalPointGain>(std::forward<const Vector3>(point), duty); }
  ...
  explicit FocalPointGain(const Vector3& point, const uint8_t duty)
      : Gain(), _point(std::forward<const Vector3>(point)), _duty(duty) {}
